...
import { DiscardQueryModal } from "../components/DiscardQueryModal";
import { getSharedSql, setSharedSql, isAutoGeneratedSql } from "../utils/queryWorkspaceStore";
...
export default function Home() {
  ...
  const [discardSqlPendingTarget, setDiscardSqlPendingTarget] = useState<{
    table: string;
    nextSql: string;
  } | null>(null);
  ...
  return (
    <React.Fragment>
      ...
      <div className="app-layout">
            ...
            <SidebarExplorer
              ...
              onSelectTable={(tbl) => {
                if (activeView === "sql") {
                  ...
                  const currentSql = getSharedSql();

                  if (!isAutoGeneratedSql(currentSql)) {
                    setDiscardSqlPendingTarget({ table: tbl, nextSql: selectSql });
                    return;
                  }
        ...
        <DiscardQueryModal
          isOpen={!!discardSqlPendingTarget}
          tableName={discardSqlPendingTarget?.table || ""}
          language={language}
          onKeepSql={() => {
            if (discardSqlPendingTarget) {
              setActiveTable(discardSqlPendingTarget.table);
              setDiscardSqlPendingTarget(null);
            }
          }}
          onDiscardAndReplace={() => {
            if (discardSqlPendingTarget) {
              setActiveTable(discardSqlPendingTarget.table);
              setSharedSql(discardSqlPendingTarget.nextSql);
              setDiscardSqlPendingTarget(null);
            }
          }}
          onCancel={() => setDiscardSqlPendingTarget(null)}
        />
